...
treatment = st.selectbox("Quel traitement souhaitez-vous appliquer ?", ["OPT Classique", "GAG - Traitement gagnants"])
...
if uploaded_files and (treatment == "GAG - Traitement gagnants" or ref_file):
    ...
    for file in uploaded_files:
        ...
        filename_base = file.name.replace(".tsv", "").replace(".xls", "").replace(".xlsx", "")
import re
match = re.search(r"OPT\s*(.*)", filename_base, re.IGNORECASE)
partner_name = match.group(1).strip() if match else "Partenaire"
out_name = f"OPT {brand_prefix} - {partner_name.upper()} GJ 2025.xlsx"
            out_file = BytesIO()

        if treatment == "OPT Classique":
